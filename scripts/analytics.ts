#!/usr/bin/env bun
/**
 * analytics — CLI for viewing gemstack skill usage statistics.
 *
 * Reads ~/.gemstack/analytics/skill-usage.jsonl and displays:
...
const ANALYTICS_FILE = path.join(os.homedir(), '.gemstack', 'analytics', 'skill-usage.jsonl');
...
export function formatReport(events: AnalyticsEvent[], period: string = 'all'): string {
  ...
  const lines: string[] = [];
  lines.push('gemstack skill usage analytics');
