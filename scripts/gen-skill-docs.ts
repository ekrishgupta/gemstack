...
echo '{"skill":"${ctx.skillName}","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.gemstack/analytics/skill-usage.jsonl 2>/dev/null || true
\`\`\`
...
**Local:** \`.gemstack/design-reports/design-audit-{domain}-{YYYY-MM-DD}.md\`

**Project-scoped:**
\`\`\`bash
source <(~/.gemini/skills/gemstack/bin/gemstack-slug 2>/dev/null) && mkdir -p ~/.gemstack/projects/$SLUG
\`\`\`
Write to: \`~/.gemstack/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md\`
...
function findTemplates(): string[] {
  ...
  const candidates = [
    ...
    path.join(ROOT, 'gemstack-upgrade', 'SKILL.md.tmpl'),
