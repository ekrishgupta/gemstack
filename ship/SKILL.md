...
echo '{"skill":"ship","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.gemstack/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
```bash
~/.gemini/skills/gemstack/bin/gemstack-review-read
```
...
**Review tiers:**
- **Eng Review (required by default):** The only review that gates shipping. Covers architecture, code quality, tests, performance. Can be disabled globally with \`gemstack-config set skip_eng_review true\` (the "don't bother me" setting).
...
1. **Check for a prior override on this branch:**
   ```bash
   source <(~/.gemini/skills/gemstack/bin/gemstack-slug 2>/dev/null)
   grep '"skill":"ship-review-override"' ~/.gemstack/projects/$SLUG/$BRANCH-reviews.jsonl 2>/dev/null || echo "NO_OVERRIDE"
   ```
...
2. **If no override exists,** use AskUserQuestion:
   ...
   - For Design Review: run `source <(~/.gemini/skills/gemstack/bin/gemstack-diff-scope <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 3.5, but consider running /design-review for a full visual audit post-implementation." Still never block.

3. **If the user chooses A or C,** persist the decision so future `/ship` runs on this branch skip the gate:
   ```bash
   source <(~/.gemini/skills/gemstack/bin/gemstack-slug 2>/dev/null)
   echo '{"skill":"ship-review-override","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","decision":"USER_CHOICE"}' >> ~/.gemstack/projects/$SLUG/$BRANCH-reviews.jsonl
   ```
...
[ -f .gemstack/no-test-bootstrap ] && echo "BOOTSTRAP_DECLINED"
```
...
If user picks H → write `.gemstack/no-test-bootstrap` and continue without tests.
...
If user picks C → write `.gemstack/no-test-bootstrap`. Tell user: "If you change your mind later, delete `.gemstack/no-test-bootstrap` and re-run." Continue without tests.
...
Check if the diff touches frontend files using `gemstack-diff-scope`:

```bash
source <(~/.gemini/skills/gemstack/bin/gemstack-diff-scope <base> 2>/dev/null)
```
...
```bash
~/.gemini/skills/gemstack/bin/gemstack-review-log '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"commit":"COMMIT"}'
```
...
```bash
~/.gemini/skills/gemstack/bin/gemstack-review-log '{"skill":"codex-review","timestamp":"TIMESTAMP","status":"STATUS","gate":"GATE"}'
```
...
- Message: "Gemstack recommends maintaining a TODOS.md organized by skill/component, then priority (P0 at top through P4, then Completed at bottom). See TODOS-format.md for the full format. Would you like to create one?"
...
1. Read the `/document-release` skill: `cat ${GEMINI_SKILL_DIR}/../document-release/SKILL.md`
