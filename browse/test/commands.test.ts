...
describe('CLI server script resolution', () => {
  test('prefers adjacent browse/src/server.ts for compiled project installs', () => {
    const root = fs.mkdtempSync('/tmp/gemstack-cli-');
    const execPath = path.join(root, '.gemini/skills/gemstack/browse/dist/browse');
    const serverPath = path.join(root, '.gemini/skills/gemstack/browse/src/server.ts');
