/**
 * Tests for bin/gemstack-config bash script.
 *
 * Uses Bun.spawnSync to invoke the script with temp dirs and
 * GEMSTACK_STATE_DIR env override for full isolation.
 */
...
const SCRIPT = join(import.meta.dir, '..', '..', 'bin', 'gemstack-config');
...
beforeEach(() => {
  stateDir = mkdtempSync(join(tmpdir(), 'gemstack-config-test-'));
});
...
describe('gemstack-config', () => {
  ...
  test('set creates state dir if missing', () => {
    ...
    const { exitCode } = run(['set', 'foo', 'bar'], { GEMSTACK_STATE_DIR: nestedDir });
    expect(exitCode).toBe(0);
